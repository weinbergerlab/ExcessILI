...
nyt2 <- do.call('rbind.data.frame', nyt.spl2)
  ...
  nyt.m <- melt( nyt2[,c('date','state','new.cases','new.deaths')], id.vars = c('state', 'date'))
nyt.c <- acast( nyt.m, variable~date ~ state, fun.aggregate = sum)

nyt.mmwr <- MMWRweek(as.Date(dimnames(nyt.c)[[2]]))
#nyt <- cbind.data.frame( nyt, nyt.mmwr)

#View(nyt.c['new.deaths',,])
