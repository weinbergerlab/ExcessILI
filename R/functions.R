...
ts_format <- function(ds1,
                      datevar, geovar, agevar,
                      syndromes,
                      resolution = "day",
                      remove.final = T) {
...
dashboardPlot <- function(all.glm.res) {
    ...
    ui <- fluidPage(titlePanel(paste0("Data through ", last.date.format)), sidebarLayout(sidebarPanel(selectInput("set.prop", 
        "Proportion of ED visits or count:", choice = c("Proportion", "Counts", 
            "Observed/Expected"), selected = "Proportion"), selectInput("set.borough", 
        "Geographic unit:", choice = counties.to.test, selected = "Citywide"), selectInput("set.syndrome", 
        "Syndrome:", choice = syndromes, selected = "ili"), checkboxInput("set.axis", 
        "Uniform axis for all plots?:", value = F), sliderInput("display.dates", 
        "Earliest date to display", min = min(dates), step = 7, max = dates[length(dates) - 
            2], value = dates[length(dates) - round(length(dates)/5)]), ), mainPanel(plotOutput("countyPlot"), 
        column(8, align = "justify", hr(), span("The black line shows the observed number of ED visits per day in the indicated stratum, and the red lines denote the mean and 95% prediction intervals for a model adjusting for seasonality, influenza activity, and RSV activity"), 
            hr(), span("This app and package were developed by The Public Health Modeling Unit and The Weinberger Lab at Yale School of Public Health. Contributors include Dan Weinberger, Alyssa Amick, Forrest Crawford, Kelsie Cassell, Marcus Rossi, Ernest Asare, Yu-Han Kao. Underlying analysis code can be found at https://github.com/weinbergerlab/ExcessILI"), 
            ))))
    shinyApp(ui, server)
}
