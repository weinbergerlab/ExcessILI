...
nrevss_flu_import <- function() {
    ...
    names(clin3) <- c("hhs_region", "wk_date", "state_name", "MMWRyear", "MMWRweek", 
        "flu_pct_pos", "hhs_percent_positive", "state")
...
reshape_ds <- function(ds2, agevar, datevar, sub.statevar) {
    ...
    form1 <- as.formula(paste(paste0(datevar, "+MMWRyear+MMWRweek"), sub.statevar, 
        agevar, "variable", sep = "~"))
...
glm.func <- function(ds, x.test, age.test, denom.var, syndrome, time.res) {
    ...
    ds.glm <- cbind.data.frame(date = date.string, day.of.year = day.of.year, y.age = y.age[1, 
        ], y.age.fit = y.age.fit, sqrt.rsv, sqrt.flu, day.of.week, t2, epiyr.index.f, 
        sin1, sin2, cos1, cos2, sin3, cos3, log.offset)
    ...
    preds.stage2 <- matrix(preds.stage2, nrow = nrow(preds.stage1.regmean), ncol = ncol(preds.stage1.regmean) * 
        5)
    ...
    out.list <- list(date = ds.glm$date, y = ds.glm$y.age, pred = preds.stage2.q[, 
        "50%"], resid1 = resid1, upi = preds.stage2.q[, "97.5%"], lpi = preds.stage2.q[, 
        "2.5%"], sqrt.rsv = ds.glm$sqrt.rsv, sqrt.flu = ds.glm$sqrt.flu, unexplained.cases = unexplained.cases, 
